...
def readCommand(argv):
    ...
    parser.add_option(
        "--test-directory",
        dest="testRoot",
        default=os.path.join(DIR_SCRIPT, "test_cases"),
        help="Root test directory which contains subdirectories corresponding to each question",
    )
    parser.add_option(
        "--student-code",
        dest="studentCode",
        help="directory where student files are located",
    )
    parser.add_option(
        "--test-class-code",
        dest="testClassRoot",
    ...
    parser.add_option(
        ...
        type=int,
        default=30,
        help="Seconds per question - otherwise timeout with fail",
    )
    parser.add_option(
        "--quietFeedback",
        action="store_true",
        help="Do not print disclaimer on feedback autograder",
    )
...
def loadModuleFile(moduleName, filePath):
    return importlib.machinery.SourceFileLoader(moduleName, filePath).load_module()
...
def runTest(testName, moduleDict, printTestCase=False, display=None, timeOut=30):
    import testParser
    import testClasses
    ...
    grades = grading.Grades(projectParams.PROJECT_NAME, [(None, 0)], timeOut=timeOut)
...
if __name__ == "__main__":
    ...
    # get the exact files to be mark from the student
    codePaths = list(
        map(
            lambda x: os.path.join(
                options.studentCode if options.studentCode else DIR_SCRIPT, x
            ),
            projectParams.STUDENT_CODE_DEFAULT,
        )
    )
    # now, actually load the student code
    moduleDict = {}
    for cp in codePaths:
        # print("Loading student file: ", cp)
        moduleName = pathlib.Path(cp).stem
        # moduleDict[moduleName] = loadModuleFile(moduleName, cp)
        try:
            moduleDict[moduleName] = func_timeout(
                1, lambda: loadModuleFile(moduleName, cp)
            )
        except FunctionTimedOut:
            print()
            print(f"Loading of student file {cp} too long, aborting import...")
            continue

    # load test classes
    moduleName = pathlib.Path(projectParams.PROJECT_TEST_CLASSES).stem
    moduleDict["projectTestClasses"] = loadModuleFile(
        moduleName,
        os.path.join(
            options.testClassRoot if options.testClassRoot else DIR_SCRIPT,
    ...
    if options.runTest != None:
        ...
        evaluate(
            ...
            quietFeedback=options.quietFeedback,
            printTestCase=options.printTestCase,
