...
class Grades:
    ...
    def __init__(
        ...
        self.quiet_feedback = quietFeedback
        self.prereqs = defaultdict(set)
        self.timeOuts = defaultdict(lambda: timeOut)

        # Better timestamp in the report
        print(f"Starting on {TIMEZONE}: {datetime.now(TIMEZONE).isoformat()}\n")
...
"""
            ...
            try:
                ...
                # OLD style
                # timed_func = timeout.TimeoutFunction(question_function, question_timeout)
                # _, time_elapsed = timed_func(self)  # call the __call__ of object

                # new style with funct_timeout()
                timed_func = util.TimeFunction(question_function, question_timeout)
                time_elapsed = func_timeout(question_timeout, lambda: timed_func(self))

                print("=====> Time taken for question: ", time_elapsed)
...
"""
        ...
        if not self.quiet_feedback:
            print()
            print(
                "\n".join(
                    textwrap.wrap(
                        "This autograding is provided to you as **feedback** information to support your progress towards your solution. As such, the results obtained may not constitute the final mark for the project, which may be  affected also by new tests and an assessment on SE quality followed. You are free (and encouraged!) to extend existing tests and build new ones. All the best!",
                        100,
                    )
                )
            )
