...
class EvalTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        result = self.evalCode(moduleDict)  # what the student got
        
        # collect all solution results starting with "result"
        # e.g., result_1 result_2 result_alt
        solution_keys = [x for x in solutionDict.keys() if x.startswith("result")]
        solutions_results = [solutionDict[key] for key in solution_keys]
        if result in solutions_results:
            ...
            grades.addMessage('\tcorrect result(s): "%s"' % solutions_results)
